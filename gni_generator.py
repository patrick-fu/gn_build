...
def generate_gni(root_path: str, gni_name: str, recursive: bool, use_related_path: bool, include_file_type=DEFAULT_TYPE) -> str:
    ...
    gni_content += '# GENERATED BY "python3 build/gni_generator.py {0} {1}{2}{3}{4}"\n'.format(
        root_path.replace('\\', '/')[len(PROJ_ROOT)+1:],
        gni_name,
        ' --no-recursive' if not recursive else '',
        ' --abs-path' if not use_related_path else '',
        ' --include-type {}'.format(' '.join(include_file_type)) if include_file_type != DEFAULT_TYPE else ''
    )
